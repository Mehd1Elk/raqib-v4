...
export default function RegulationCard({ data }: { data: Record<string, any> }) {
  return (
    <div data-card className="bg-[#FDFAF3] border border-[rgba(60,52,40,0.10)] rounded-lg p-4 border-l-[3px]" style={{ borderLeftColor: '#3D5E8C' }}>
      <div className="flex items-start gap-3">
        <Scale size={16} className="text-[#3D5E8C] mt-0.5 flex-shrink-0" />
        <div>
          <div className="font-[family-name:var(--font-cormorant)] text-[13px] font-bold italic text-[#1C1814]">{data.loi || data.article || data.titre}</div>
          ...
          {data.pertinence_AELYA && (
            <div className="font-[family-name:var(--font-noto)] text-[9px] text-[#6B5E4C] mt-2 italic">{data.pertinence_AELYA}</div>
          )}
        </div>
      </div>
    </div>
