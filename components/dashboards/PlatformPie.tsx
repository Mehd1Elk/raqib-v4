...
export function PlatformPie({ data }: Props) {
  const chartData = data.map((d) => ({
    ...
    color: PLATFORMS[d.platform_code as keyof typeof PLATFORMS]?.color ?? 'rgba(30,10,32,0.60)',
  }));
    ...
    <div className="w-full h-[240px] flex items-center">
      <div className="flex-1 h-full">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={chartData} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={80} innerRadius={40} strokeWidth={1} stroke="#EEEBF4">
      ...
      <div className="w-36 space-y-1.5">
        {chartData.map((d) => (
          <div key={d.platform_code} className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-none-full shrink-0" style={{ background: d.color }} />
