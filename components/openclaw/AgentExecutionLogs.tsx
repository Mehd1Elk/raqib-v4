    ...
    <div className="bg-[#FDFAF3] border border-[rgba(60,52,40,0.10)] rounded-lg p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <History size={14} className="text-[#B8963E]" />
        ...
        <div className="flex gap-4 items-center">
          <span className="font-['JetBrains_Mono'] text-[8px] text-[#918977]">
            {logs.length} runs
          </span>
          <span className="font-['JetBrains_Mono'] text-[8px] text-[#918977] flex items-center gap-1">
            {totalTokens.toLocaleString()} tok
            <DollarSign size={8} className="text-[#B8963E]" />
            <span className="text-[#B8963E]">~${estimatedCost.toFixed(3)}</span>
          </span>
        ...
        <div className="mb-3 bg-[#1C1814] rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowReplay(false)}
              className="font-['JetBrains_Mono'] text-[8px] text-[#918977] hover:text-[#F7F3EA]">
              FERMER
            </button>
          </div>
          <div className="flex gap-4 font-['JetBrains_Mono'] text-[8px] text-[#918977]">
          ...
          <div>
            <div className="font-['JetBrains_Mono'] text-[7px] text-[#918977] mb-1">
              PROMPT
            </div>
            <pre className="font-['JetBrains_Mono'] text-[9px] text-[#D4B662] whitespace-pre-wrap max-h-[200px] overflow-y-auto">
              {replay.prompt}
            </pre>
          </div>
      ...
      {showDiff && lastSuccess && lastError && (
        <div className="mb-3 grid grid-cols-2 gap-2">
          <div className="bg-[#1C1814] rounded p-3">
            ...
            <pre className="font-['JetBrains_Mono'] text-[8px] text-[#F7F3EA] whitespace-pre-wrap max-h-[150px] overflow-y-auto">
              {lastSuccess.output}
            </pre>
          </div>
          <div className="bg-[#1C1814] rounded p-3">
          ...
          <div key={log.id}>
            <div className="flex items-center gap-3 py-1.5 px-2 rounded hover:bg-[rgba(184,150,62,0.03)] cursor-pointer"
              ...
              {expanded === log.id
                ...
                : <ChevronRight size={10} className="text-[#918977]" />}
              {log.status === 'success'
                ? <Check size={10} className="text-[#3D7C5E]" />
                : <X size={10} className="text-[#9C3D3D]" />}
              <span className="font-['JetBrains_Mono'] text-[8px] text-[#918977] w-28">
                {new Date(log.timestamp).toLocaleString('fr-FR', {
                  day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit'
                })}
              </span>
              <span className="font-['JetBrains_Mono'] text-[8px] text-[#918977] w-12">
                {log.duration}
              </span>
              <span className="font-['JetBrains_Mono'] text-[8px] text-[#918977] w-20">
                {log.tokens.toLocaleString()} tok
                <span className="text-[#B8963E] ml-1">
            ...
            {expanded === log.id && (
              <div className="ml-8 mb-2 p-3 bg-[#1C1814] rounded">
                ...
                <div className="font-['JetBrains_Mono'] text-[7px] text-[#918977] mb-1">OUTPUT</div>
