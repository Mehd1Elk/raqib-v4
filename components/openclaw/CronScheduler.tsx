...
const LAYER_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  'L1.5': { bg: '#EEEBF4', text: '#1E0A20' },
  L2:     { bg: '#E4E0EE', text: '#1E0A20' },
  L3:     { bg: '#EEEBF4', text: 'rgba(30,10,32,0.60)' },
  OPS:    { bg: 'rgba(30,10,32,0.45)', text: '#FAF8FC' },
  CORE:   { bg: '#1E0A20', text: '#FAF8FC' },
};
...
export function CronScheduler() {
  ...
  const layerColor = (layer: string) =>
    LAYER_COLORS[layer] ?? { bg: '#E4E0EE', text: '#1E0A20' };

  return (
    <div className="w-full h-full flex flex-col bg-[#FAF8FC] overflow-hidden">

      {/* Header */}
      <div className="flex items-center justify-between px-6 py-3 border-b border-[rgba(30,10,32,0.08)] shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="font-[family-name:var(--font-jetbrains)] text-[9px] text-[rgba(30,10,32,0.45)]">
          ...
          <span
            className={`font-[family-name:var(--font-jetbrains)] text-[7px] px-1.5 py-[2px] rounded-none-none ${
              source === 'LIVE'
                ...
                : 'bg-[#EEEBF4] text-[rgba(30,10,32,0.45)]'
            }`}
          >
        ...
        <button
          ...
          className="p-1 hover:bg-[#EEEBF4] rounded-none transition-colors"
          aria-label="Actualiser"
        >
          <RefreshCw
            size={12}
            className={loading ? 'animate-spin text-[#1E0A20]' : 'text-[rgba(30,10,32,0.45)]'}
          />
        </button>
      </div>

      {/* Layer filter pills */}
      <div className="flex items-center gap-1.5 px-6 py-2.5 border-b border-[rgba(30,10,32,0.08)] overflow-x-auto shrink-0">
        {layers.map(layer => {
            ...
            <button
              ...
              style={
                isActive && lc
                  ...
                  : { backgroundColor: '#EEEBF4', color: 'rgba(30,10,32,0.45)' }
              }
            >
              {layer}
            </button>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          <div className="p-4 space-y-2">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="flex items-center gap-4 px-4 py-3 animate-pulse">
                <div className="w-2 h-2 rounded-none-none bg-[rgba(30,10,32,0.08)]" />
                <div className="h-3 w-12 rounded-none bg-[rgba(30,10,32,0.08)]" />
                <div className="h-3 flex-1 rounded-none bg-[rgba(30,10,32,0.08)]" />
                <div className="h-3 w-10 rounded-none bg-[rgba(30,10,32,0.08)]" />
                <div className="h-3 w-16 rounded-none bg-[rgba(30,10,32,0.08)]" />
              </div>
            ))}
          </div>
        ) : filteredJobs.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 gap-3">
            <Clock size={20} className="text-[rgba(30,10,32,0.18)]" />
            <span className="font-[family-name:var(--font-jetbrains)] text-[9px] text-[rgba(30,10,32,0.45)] uppercase tracking-wider">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-[rgba(30,10,32,0.08)]">
                {['', 'Couche', 'Agent', 'Freq', 'Prochain run', 'Actions'].map((h, i) => (
                  <th
                    key={i}
                    className="px-4 py-2 text-left font-[family-name:var(--font-jetbrains)] text-[8px] uppercase tracking-wider text-[rgba(30,10,32,0.45)] font-normal"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {filteredJobs.map(job => {
                ...
                return (
                  <tr
                    key={job.agentId}
                    className="border-b border-[rgba(30,10,32,0.04)] hover:bg-[#EEEBF4] transition-colors"
                  >
                    {/* Status dot */}
                    <td className="pl-4 pr-2 py-3 w-6">
                      <span
                        className={`w-2 h-2 rounded-none-none inline-block ${STATUS_DOT[job.status] ?? 'bg-[rgba(30,10,32,0.45)]'}`}
                      />
                    </td>
                    ...
                    <td className="px-3 py-3 min-w-[160px]">
                      ...
                      <div className="font-[family-name:var(--font-jetbrains)] text-[7px] text-[rgba(30,10,32,0.45)] mt-0.5">
                        {job.agentId}
                      </div>
                    </td>
                    ...
                    {/* Countdown */}
                    <td className="px-3 py-3 whitespace-nowrap">
                      <span className="font-[family-name:var(--font-jetbrains)] text-[9px] text-[rgba(30,10,32,0.70)]">
