...
import { generateTickerData, type TickerCategory } from '../shared/mock-data';
...
export default function IntentionBourseView() {
  const [categoriesData, setCategoriesData] = useState<TickerCategory[]>(generateTickerData());
  const [selectedCat, setSelectedCat] = useState(categoriesData[0]);

  // Refresh ticker data every 8 seconds for live demo effect
  useEffect(() => {
    const interval = setInterval(() => {
      const fresh = generateTickerData();
      setCategoriesData(fresh);
    }, 8000);
