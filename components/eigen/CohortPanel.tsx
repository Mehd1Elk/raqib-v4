...
export default function CohortPanel({ cohortId, cohortName, cohortColor, agents, description, onClose }: CohortPanelProps) {
  ...
  return (
    <div className="bg-[#FAF8FC] border border-[#E5E0D8] rounded-none-none overflow-hidden h-full flex flex-col shadow-xl">
      {/* Header */}
          ...
          <div className="border-t border-[#E5E0D8] p-4 bg-[#FDFCFB]">
            <div className="flex items-center gap-3">
              <input
                ...
                className="flex-1 font-sans text-sm bg-white border border-[#E5E0D8] rounded-none px-4 py-2.5 focus:border-[#1E0A20] focus:ring-1 focus:ring-[#1E0A20] outline-none shadow-sm transition-all text-stone-800"
              ...
              <button 
                ...
                className="px-4 py-2.5 bg-[#1E0A20] text-white rounded-none font-mono text-[10px] font-bold uppercase hover:bg-[#1E0A20] transition-colors disabled:opacity-40 disabled:hover:bg-[#1E0A20] flex items-center gap-2"
              >
            ...
            <div className="flex gap-2 mt-3 flex-wrap">
              {getSuggestedTopics(cohortId).map(s => (
                <button 
                  key={s} 
                  onClick={() => { setTopic(s); }} 
                  className="font-mono text-[9px] px-3 py-1 bg-stone-100 border border-stone-200 text-stone-600 rounded-none-full hover:text-[#1E0A20] hover:border-[#1E0A20] hover:bg-white transition-all"
                >
                  {s}
                </button>
              ))}
...
function AgentMessage({ message, cohortColor }: { message: Message; cohortColor: string }) {
  if (message.agentId === 'system') {
    return (
      <div className="text-center py-2 animate-in fade-in zoom-in duration-300">
        <span className="font-mono text-[10px] uppercase font-bold tracking-wider text-stone-500 bg-stone-100 border border-stone-200 px-4 py-1.5 rounded-none-full shadow-sm">
          {message.content}
        </span>
      </div>
    );
  ...
  return (
    <div className="flex gap-3 animate-in slide-in-from-bottom-2 fade-in duration-300">
      <div className="w-8 h-8 rounded-none-full flex items-center justify-center flex-shrink-0 mt-1 shadow-sm" style={{ backgroundColor: cohortColor + '15', border: `1px solid ${cohortColor}40` }}>
        <Cpu size={14} style={{ color: cohortColor }} />
      </div>
      <div className="flex-1 min-w-0 bg-stone-50 border border-stone-100 rounded-none-none rounded-none-tl-none p-3 shadow-sm">
        ...
        <div className="font-sans text-sm text-stone-700 leading-relaxed">
          {message.content.split(/(@[\w-]+)/g).map((part, i) => {
             if (part.startsWith('@')) return <span key={i} className="text-[#1E0A20] font-mono text-[11px] font-bold bg-[#1E0A20]/10 px-1 rounded-none">{part}</span>;
             return part;
          })}
        </div>
      </div>
...
function TypingIndicator() {
  return (
    <div className="flex items-center gap-3 py-2 animate-in fade-in duration-300">
      <div className="w-8 h-8 rounded-none-full bg-stone-100 flex items-center justify-center border border-stone-200">
        <div className="flex gap-1">
          <span className="w-1.5 h-1.5 rounded-none-full bg-stone-400 animate-bounce" style={{ animationDelay: '0ms' }} />
          <span className="w-1.5 h-1.5 rounded-none-full bg-stone-400 animate-bounce" style={{ animationDelay: '150ms' }} />
          <span className="w-1.5 h-1.5 rounded-none-full bg-stone-400 animate-bounce" style={{ animationDelay: '300ms' }} />
        </div>
      </div>
