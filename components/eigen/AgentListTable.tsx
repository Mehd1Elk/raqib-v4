...
export const AgentListTable: React.FC<AgentListTableProps> = ({ data, onRowClick }) => {
  ...
  const columns = [
    ...
    columnHelper.accessor('layer', {
      ...
      cell: info => <span className="text-[10px] font-mono px-2 py-0.5 bg-stone-100 rounded-none text-[#1E0A20]">{info.getValue()}</span>,
    }),
    ...
    columnHelper.accessor('status', {
      ...
      cell: info => {
        ...
        return <span className={`text-[10px] font-semibold px-2 py-1 rounded-none-full ${color}`}>{status}</span>;
      },
    }),
    ...
    <div className="w-full bg-white shadow-sm border border-stone-200 rounded-none overflow-hidden">
      <div className="flex justify-between items-center p-3 border-b border-stone-200 bg-stone-50">
        ...
        <button 
          ...
          className="flex items-center space-x-2 px-3 py-1.5 bg-[#1E0A20] text-white text-xs font-semibold rounded-none hover:bg-[#C5A028] transition-colors"
        >
          <Download size={14} />
      ...
      <div className="overflow-x-auto">
        <table aria-label="Liste des agents EIGEN" className="min-w-full divide-y divide-stone-200 text-left text-sm">
          <thead className="bg-[#F5F2F8] text-[#1E0A20]">
