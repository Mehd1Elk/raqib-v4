...
const TYPE_STYLES: Record<string, { bg: string; text: string; border: string }> = {
  docx: { bg: 'bg-[#3D5E8C10]', text: 'text-[#5A6E9C]', border: 'border-[#3D5E8C30]' },
  jsx: { bg: 'bg-[#7B5EA710]', text: 'text-[#8B5EB0]', border: 'border-[#7B5EA730]' },
  html: { bg: 'bg-[#3D7C5E10]', text: 'text-[#5A8A6E]', border: 'border-[#3D7C5E30]' },
  antigravity: { bg: 'bg-[#9C3D3D10]', text: 'text-[#8C3040]', border: 'border-[#9C3D3D30]' },
};
    ...
    <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-[#FAF8FC] rounded-none-none w-[480px] max-h-[80vh] overflow-auto shadow-xl">
        <div className="flex items-center justify-between px-5 py-3 border-b border-[rgba(30,10,32,0.08)]">
          <span className="font-[family-name:var(--font-cormorant)] text-[16px] font-bold ">Ajouter à la galerie</span>
          <button onClick={onClose}><X size={16} className="text-[rgba(30,10,32,0.45)]" /></button>
        </div>
        <div className="p-5 space-y-4">
          <div className="flex items-center gap-3 p-3 bg-[#EEEBF4] rounded-none-none">
            ...
            <div>
              ...
              <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.45)]">{(file.size / 1024).toFixed(1)} KB</div>
            </div>
          </div>
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Titre"
            className="w-full font-[family-name:var(--font-cormorant)] text-[14px]  border-b border-[rgba(30,10,32,0.08)] pb-1 bg-transparent focus:border-[#1E0A20] outline-none" />
          <select value={category} onChange={e => setCategory(e.target.value)}
            className="w-full font-[family-name:var(--font-jetbrains)] text-[10px] text-[rgba(30,10,32,0.45)] border border-[rgba(30,10,32,0.08)] rounded-none px-2 py-1.5 bg-transparent focus:border-[#1E0A20] outline-none">
            <option value="">Catégorie...</option>
            {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
          <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Description..."
            className="w-full font-[family-name:var(--font-noto)] text-[11px] border border-[rgba(30,10,32,0.08)] rounded-none px-2 py-1.5 bg-transparent h-16 resize-none focus:border-[#1E0A20] outline-none" />
        </div>
        <div className="flex justify-end gap-2 px-5 py-3 border-t border-[rgba(30,10,32,0.08)]">
          <button onClick={onClose} className="px-4 py-2 font-[family-name:var(--font-jetbrains)] text-[10px] text-[rgba(30,10,32,0.45)]">Annuler</button>
          <button onClick={() => onConfirm({ title, category, description })}
            className="px-4 py-2 bg-[#1E0A20] text-white font-[family-name:var(--font-jetbrains)] text-[10px] rounded-none hover:bg-[rgba(30,10,32,0.80)] transition">
            Ajouter
          </button>
        </div>
      </div>
...
function EditModal({ item, onClose, onSave }: { item: GalleryItem; onClose: () => void; onSave: (updated: Partial<GalleryItem>) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-[#FAF8FC] rounded-none-none w-[480px] max-h-[80vh] overflow-auto shadow-xl">
        <div className="flex items-center justify-between px-5 py-3 border-b border-[rgba(30,10,32,0.08)]">
          <span className="font-[family-name:var(--font-cormorant)] text-[16px] font-bold ">Modifier le document</span>
          <button onClick={onClose}><X size={16} className="text-[rgba(30,10,32,0.45)]" /></button>
        </div>
        <div className="p-5 space-y-4">
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Titre"
...
function CardContextMenu({ onEdit, onDuplicate, onDelete }: { onEdit: () => void; onDuplicate: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="relative">
      <button onClick={e => { e.stopPropagation(); setOpen(!open); }} className="p-1 rounded-none hover:bg-[rgba(30,10,32,0.05)]">
        <MoreVertical size={14} className="text-[rgba(30,10,32,0.45)]" />
      </button>
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 w-[160px] bg-[#FAF8FC] border border-[rgba(30,10,32,0.08)] rounded-none-none shadow-lg z-30 py-1">
            <button onClick={e => { e.stopPropagation(); onEdit(); setOpen(false); }} className="w-full text-left px-3 py-1.5 flex items-center gap-2 hover:bg-[rgba(30,10,32,0.05)] font-[family-name:var(--font-noto)] text-[10px] text-[rgba(30,10,32,0.55)]"><Pencil size={11} /> Modifier</button>
            <button onClick={e => { e.stopPropagation(); onDuplicate(); setOpen(false); }} className="w-full text-left px-3 py-1.5 flex items-center gap-2 hover:bg-[rgba(30,10,32,0.05)] font-[family-name:var(--font-noto)] text-[10px] text-[rgba(30,10,32,0.55)]"><Copy size={11} /> Dupliquer</button>
            <div className="border-t border-[rgba(30,10,32,0.08)] my-1" />
            <button onClick={e => { e.stopPropagation(); onDelete(); setOpen(false); }} className="w-full text-left px-3 py-1.5 flex items-center gap-2 hover:bg-[rgba(156,61,61,0.05)] text-[#8C3040] font-[family-name:var(--font-noto)] text-[10px]"><Trash2 size={11} /> Supprimer</button>
          </div>
        </>
      )}
    </div>
...
function VersionHistory({ files }: { files: string[] }) {
  return (
    <div className="mt-3 pt-3 border-t border-[rgba(30,10,32,0.08)]">
      <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.45)] tracking-wider mb-2">HISTORIQUE DES VERSIONS</div>
      {files.map((filename, i) => (
        <div key={filename} className="flex items-center justify-between py-1.5">
          ...
          <span className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.45)]">{i === 0 ? 'Actuelle' : 'Archivée'}</span>
        </div>
      ))}
    </div>
  );
...
function ArtifactThumbnail({ item }: { item: GalleryItem }) {
  ...
  if ((item.type === 'jsx' || item.type === 'html') && item.filename) {
    return (
      <div className="w-full h-[140px] overflow-hidden bg-[#EEEBF4] relative">
        <iframe
  ...
  return (
    <div className={`w-full h-[140px] ${style.bg} flex flex-col items-center justify-center relative`}>
      ...
      <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.45)] mt-2">{item.type.toUpperCase()}</div>
      <div className={`absolute top-2 right-2 px-2 py-0.5 ${style.bg} ${style.text} font-[family-name:var(--font-jetbrains)] text-[7px] rounded-none border ${style.border}`}>
        {item.type.toUpperCase()}
      </div>
    </div>
...
export function EigenGallery() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#FAF8FC]" onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
      {/* Drag overlay */}
      {dragging && (
        <div className="fixed inset-0 z-40 bg-[rgba(30,10,32,0.05)] border-2 border-dashed border-[#1E0A20] flex items-center justify-center pointer-events-none">
      ...
      {/* Top Bar */}
      <div className="flex flex-wrap items-center justify-between px-6 py-3 border-b border-[rgba(30,10,32,0.08)] bg-[#FAF8FC] gap-3 sticky top-0 z-10">
        <div className="flex items-center gap-3 flex-1">
          ...
          <a href="/vault" className="flex items-center gap-1.5 px-3 py-1.5 border border-[#8B5EB0] text-[#8B5EB0] rounded-none font-[family-name:var(--font-jetbrains)] text-[9px] hover:bg-[#8B5EB0] hover:text-white transition">
            <Archive size={12} /> VAULT — 111
          </a>

          <div className="relative flex-1 max-w-[220px]">
            <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[rgba(30,10,32,0.45)]" />
            <input type="text" placeholder="Rechercher..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full bg-[#EEEBF4] border border-[rgba(30,10,32,0.08)] text-[10px] font-[family-name:var(--font-jetbrains)] pl-8 pr-2 py-1.5 outline-none rounded-none focus:border-[#1E0A20] placeholder-[rgba(30,10,32,0.35)]" />
          </div>

          <select value={filterCat} onChange={e => setFilterCat(e.target.value)}
            className="bg-[#EEEBF4] border border-[rgba(30,10,32,0.08)] text-[#1E0A20] text-[10px] font-[family-name:var(--font-jetbrains)] px-2 py-1.5 outline-none rounded-none">
        ...
        <div className="flex items-center gap-2">
          <span className="font-[family-name:var(--font-jetbrains)] text-[9px] text-[rgba(30,10,32,0.45)]">{filtered.length} doc{filtered.length > 1 ? 's' : ''}</span>
          <div className="flex border border-[rgba(30,10,32,0.08)] rounded-none overflow-hidden">
            <button onClick={() => setViewMode('grid')}
              className={`p-1.5 transition-colors ${viewMode === 'grid' ? 'bg-[#1E0A20] text-white' : 'text-[rgba(30,10,32,0.45)] hover:bg-[#EEEBF4]'}`}>
              <LayoutGrid size={14} />
            </button>
            <button onClick={() => setViewMode('list')}
              className={`p-1.5 transition-colors ${viewMode === 'list' ? 'bg-[#1E0A20] text-white' : 'text-[rgba(30,10,32,0.45)] hover:bg-[#EEEBF4]'}`}>
              <Rows3 size={14} />
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {filtered.map(item => {
              ...
              return (
                <div key={item.id}
                  className="bg-[#FAF8FC] border border-[rgba(30,10,32,0.08)] rounded-none-none overflow-hidden flex flex-col transition-all duration-200 hover:shadow-md hover:border-[#1E0A20] group">
                  <div className="cursor-pointer" onClick={() => handleCardClick(item)}>
                    <ArtifactThumbnail item={item} />
                  </div>
                  <div className="p-3">
                    <div className="flex items-start justify-between gap-1 mb-2">
                      <h4 className="font-[family-name:var(--font-cormorant)] text-[13px] font-bold  text-[#1E0A20] leading-snug cursor-pointer hover:text-[#1E0A20] transition-colors flex-1"
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-2">
                      <span className="font-[family-name:var(--font-jetbrains)] text-[8px] px-1.5 py-0.5 bg-[#EEEBF4] text-[rgba(30,10,32,0.45)] rounded-none">{item.category}</span>
                      {vg && (
                        <span className="font-[family-name:var(--font-jetbrains)] text-[7px] px-1.5 py-0.5 bg-[rgba(90,110,156,0.12)] text-[#5A6E9C] rounded-none">
                          V{vg.files.length} · {vg.files.length} versions
                        </span>
                      )}
                    </div>
                    <div className="flex items-center justify-between text-[rgba(30,10,32,0.45)] text-[8px] font-[family-name:var(--font-jetbrains)]">
                      <span>{item.date}</span>
                      <span>{item.size}</span>
                    </div>
                  </div>
          ...
          <div className="space-y-2">
            {filtered.map(item => {
              ...
              return (
                <div key={item.id}
                  onClick={() => handleCardClick(item)}
                  className="flex items-center gap-4 px-4 py-3 bg-[#FAF8FC] border border-[rgba(30,10,32,0.08)] rounded-none-none hover:border-[#1E0A20] transition cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-[family-name:var(--font-cormorant)] text-[13px] font-bold  text-[#1E0A20] truncate">{item.title}</span>
                      {vg && <span className="font-[family-name:var(--font-jetbrains)] text-[7px] px-1 py-0.5 bg-[rgba(90,110,156,0.12)] text-[#5A6E9C] rounded-none shrink-0">V{vg.files.length}</span>}
                    </div>
                    <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.45)]">{item.category}</div>
                  </div>
                  <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.45)]">{item.date}</div>
                  <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.45)]">{item.size}</div>
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center text-[rgba(30,10,32,0.45)] text-[11px] font-[family-name:var(--font-noto)]">Aucun document trouvé.</div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-8" onClick={() => setViewer(null)}>
          <div className="bg-[#FAF8FC] rounded-none-none w-full max-w-[900px] h-[80vh] flex flex-col overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-[rgba(30,10,32,0.08)]">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-[family-name:var(--font-cormorant)] text-[16px] font-bold  text-[#1E0A20]">{viewer.title}</h3>
                  {(() => { const vg = getVersionGroup(viewer.filename); return vg ? <span className="font-[family-name:var(--font-jetbrains)] text-[7px] px-1.5 py-0.5 bg-[rgba(90,110,156,0.12)] text-[#5A6E9C] rounded-none">V{vg.files.length}</span> : null; })()}
                </div>
                <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.45)]">{viewer.category} · {viewer.date} · {viewer.size}</div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => window.open(`/artifacts/${viewer.filename}`, '_blank')}
                  className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.45)] hover:text-[#1E0A20] px-2 py-1 border border-[rgba(30,10,32,0.08)] rounded-none transition-colors">
                  OUVRIR SÉPARÉMENT
                </button>
                <button onClick={() => setViewer(null)} className="p-1 hover:bg-[#EEEBF4] rounded-none text-[rgba(30,10,32,0.45)]">
                  <X size={16} />
                </button>
              </div>
            </div>
            <div className="flex-1 overflow-auto bg-[#EEEBF4]">
              {viewer.filename ? (
                <ArtifactViewer artifactName={viewer.filename} height={typeof window !== 'undefined' ? window.innerHeight * 0.65 : 500} />
              ) : (
                <div className="flex items-center justify-center h-full text-[rgba(30,10,32,0.45)] font-[family-name:var(--font-jetbrains)] text-[11px]">
                  Aperçu non disponible pour ce type de document
                </div>
              )}
            </div>
            {/* Version History in viewer */}
            {(() => { const vg = getVersionGroup(viewer.filename); return vg ? <div className="px-5 py-3 border-t border-[rgba(30,10,32,0.08)]"><VersionHistory files={vg.files} /></div> : null; })()}
          </div>
        </div>
      )}
    </div>
