...
const SEED_CARDS: ConquestCard[] = [
  ...
  { id: 'seed-2', title: 'GITEX Africa Marrakech', columnStatus: 'in_progress', deadline: '7 avril', priority: 'P0', responsible: 'EE', entity: 'EIGEN', entityColor: '#1E0A20', progress: 75, description: 'Stand, pitch deck, démos NOOS + BURHAN.', blockers: ['Badge exposant en attente'], notes: '', validated: false, position: 1 },
  ...
  { id: 'seed-6', title: 'VivaTech Paris', columnStatus: 'planned', deadline: 'juin', priority: 'P2', responsible: 'YB', entity: 'EIGEN', entityColor: '#1E0A20', progress: 0, description: '', blockers: [], notes: '', validated: false, position: 5 },
...
const COLUMNS: { id: UIStatus; dbStatus: ColumnStatus; label: string; color: string; bg: string }[] = [
  { id: 'PLANNING', dbStatus: 'planned', label: 'À PLANIFIER', color: 'rgba(30,10,32,0.60)', bg: 'bg-[#F2EFE8]' },
  { id: 'IN_PROGRESS', dbStatus: 'in_progress', label: 'EN COURS', color: '#1E0A20', bg: 'bg-[#FCF5E3]' },
...
const PRIORITY_STYLES: Record<Priority, string> = {
  ...
  P2: 'text-[rgba(30,10,32,0.60)] bg-[rgba(30,10,32,0.60)10] border-[rgba(30,10,32,0.60)30]',
};
...
function ConquestCardUI({ card, onUpdate, onDelete, onDragStart }: {
  ...
  const progressColor = card.progress === 100 ? '#3D7C5E' : card.progress > 50 ? '#1E0A20' : '#B87D3E';
  ...
  return (
    <div
      ...
      className={`bg-white border rounded-none-none overflow-hidden transition-all hover:shadow-md cursor-grab active:cursor-grabbing ${
        card.validated ? 'opacity-60 border-[#3D7C5E40]' : card.priority === 'P0' ? 'border-[#9C3D3D30]' : 'border-[rgba(30,10,32,0.35)]'
      }`}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-[rgba(30,10,32,0.35)]">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <span className="w-2 h-2 rounded-none-full shrink-0"
            style={{ backgroundColor: card.priority === 'P0' ? '#9C3D3D' : card.priority === 'P1' ? '#B87D3E' : 'rgba(30,10,32,0.60)' }} />
          {editingTitle ? (
            <input
              ...
              autoFocus
              className="font-[family-name:var(--font-cormorant)] text-[12px] font-bold italic bg-transparent border-b border-[#1E0A20] outline-none w-full"
            />
          ) : (
            <span
        ...
        <div className="flex items-center gap-1 shrink-0 ml-2">
          ...
          <button onClick={e => { e.stopPropagation(); setExpanded(!expanded); }} className="p-0.5 hover:text-[#1E0A20]">
            <ChevronDown size={12} strokeWidth={1.5} className={`text-[rgba(30,10,32,0.60)] transition ${expanded ? 'rotate-180' : ''}`} />
          </button>
        </div>
      </div>
      ...
      <div className="px-3 py-2">
        {card.deadline && (
          <div className="flex items-center gap-1 mb-1.5">
            <Calendar size={10} strokeWidth={1.5} className="text-[rgba(30,10,32,0.60)]" />
            <span className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.60)]">{card.deadline}</span>
          </div>
        )}
        <div className="flex items-center gap-2 mb-2">
          <span className="font-[family-name:var(--font-jetbrains)] text-[8px] bg-[#F2EFE8] text-[#1E0A20] px-1.5 py-0.5 rounded-none border border-[rgba(30,10,32,0.35)] font-bold">{card.responsible}</span>
          {card.entity && (
            <span className="font-[family-name:var(--font-jetbrains)] text-[7px] px-1.5 py-0.5 rounded-none"
              style={{ backgroundColor: (card.entityColor || 'rgba(30,10,32,0.60)') + '15', color: card.entityColor }}>
              {card.entity}
            </span>
          )}
        ...
        {/* Progress */}
        <div className="w-full h-1.5 bg-[#F2EFE8] rounded-none-full overflow-hidden">
          <div className="h-full rounded-none-full transition-all duration-500" style={{ width: `${card.progress}%`, backgroundColor: progressColor }} />
        </div>
        <div className="font-[family-name:var(--font-jetbrains)] text-[7px] text-[rgba(30,10,32,0.60)] mt-0.5 text-right">{card.progress}%</div>
      </div>

      {/* Expanded panel */}
      {expanded && (
        <div className="px-3 py-3 border-t border-[rgba(30,10,32,0.35)] bg-[#F5F2F8]">
          {/* Description */}
          <textarea
            defaultValue={card.description}
            placeholder="Ajouter une description..."
            onBlur={e => onUpdate({ ...card, description: e.target.value })}
            className="w-full font-[family-name:var(--font-noto)] text-[10px] bg-transparent border border-[rgba(30,10,32,0.35)] rounded-none p-2 min-h-[50px] resize-none focus:border-[#1E0A20] outline-none mb-2"
          />

          {/* Progress slider */}
          <div className="mb-2">
            <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.60)] tracking-wider mb-1">PROGRESSION</div>
          ...
          {/* Priority select */}
          <div className="mb-2">
            <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.60)] tracking-wider mb-1">PRIORITÉ</div>
            <div className="flex gap-1">
              {(['P0', 'P1', 'P2'] as Priority[]).map(p => (
                <button key={p} onClick={() => onUpdate({ ...card, priority: p })}
                  className={`px-2 py-0.5 rounded-none text-[8px] font-[family-name:var(--font-jetbrains)] border transition ${card.priority === p ? PRIORITY_STYLES[p] + ' font-bold' : 'border-[rgba(30,10,32,0.35)] text-[rgba(30,10,32,0.60)]'}`}>
                  {p}
                </button>
              ))}
            </div>
          ...
          {/* Blockers */}
          <div className="mb-2">
            <div className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[rgba(30,10,32,0.60)] tracking-wider mb-1">BLOCKERS</div>
            {card.blockers.map((blocker, i) => (
              <div key={i} className="flex items-center gap-1 mb-1">
                ...
                <input
                  ...
                  className="flex-1 font-[family-name:var(--font-noto)] text-[9px] bg-transparent border-b border-[rgba(30,10,32,0.35)] focus:border-[#9C3D3D] outline-none"
                />
                <button onClick={() => onUpdate({ ...card, blockers: card.blockers.filter((_, j) => j !== i) })}>
                  <X size={10} className="text-[rgba(30,10,32,0.60)] hover:text-[#9C3D3D]" />
                </button>
              </div>
            ))}
            <button onClick={() => onUpdate({ ...card, blockers: [...card.blockers, ''] })}
              className="font-[family-name:var(--font-jetbrains)] text-[8px] text-[#1E0A20] hover:underline">
              + Ajouter un blocker
            </button>
          </div>
          ...
          <textarea
            ...
            className="w-full font-[family-name:var(--font-noto)] text-[9px] bg-[#FAF8FC] border border-[rgba(30,10,32,0.35)] rounded-none p-2 min-h-[40px] resize-none focus:border-[#1E0A20] outline-none mb-2"
          />

          {/* Actions */}
          <div className="flex items-center gap-2 pt-2 border-t border-[rgba(30,10,32,0.35)]">
            {!card.validated ? (
...
export function EigenConquest() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full bg-[#FAF8FC]">
        ...
        <span className="ml-2 font-[family-name:var(--font-jetbrains)] text-[11px] text-[rgba(30,10,32,0.60)]">Chargement des jalons...</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[#FAF8FC]">
      {/* Header */}
      <div className="shrink-0 px-6 py-3 border-b border-[rgba(30,10,32,0.35)] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-[family-name:var(--font-jetbrains)] text-[9px] text-[rgba(30,10,32,0.60)]">{cards.length} jalons</span>
      ...
      <div className="flex flex-1 gap-4 overflow-x-auto p-5">
        {COLUMNS.map(col => {
          ...
          return (
            <div key={col.id}
              className={`flex-1 min-w-[280px] max-w-[340px] flex flex-col border border-[rgba(30,10,32,0.35)] rounded-none-none overflow-hidden ${col.bg} ${draggedId ? 'ring-1 ring-dashed ring-[#1E0A2030]' : ''}`}
              onDragOver={e => e.preventDefault()}
              onDrop={() => handleDrop(col.dbStatus)}>
              <div className="px-4 py-3 border-b border-[rgba(30,10,32,0.35)] bg-white flex justify-between items-center" style={{ borderTop: `2px solid ${col.color}` }}>
                ...
                <span className="font-[family-name:var(--font-jetbrains)] text-[8px] bg-[#F2EFE8] px-2 py-0.5 border border-[rgba(30,10,32,0.35)] rounded-none-full text-[rgba(30,10,32,0.60)]">{colCards.length}</span>
              </div>
              <div className="flex-1 p-3 flex flex-col gap-3 overflow-y-auto">
                ...
                {colCards.length === 0 && (
                  <div className="text-center py-6 text-[rgba(30,10,32,0.60)] font-[family-name:var(--font-jetbrains)] text-[9px]">
                    Aucun jalon
                  </div>
                )}
              </div>
