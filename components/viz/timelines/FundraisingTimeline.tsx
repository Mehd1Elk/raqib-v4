  ...
  return (
    <div className="w-full bg-[#111111] overflow-x-auto p-6 md:p-12 border border-stone-800 custom-scrollbar">
      <div className="relative flex min-w-max items-center h-64">
        {/* Ligne directrice */}
        <div className="absolute top-[30%] left-0 right-0 h-1 bg-stone-900 rounded-none-full"></div>
        <div className="absolute top-[30%] left-0 right-0 h-1 bg-gradient-to-r from-[#1E0A20]/20 via-[#1E0A20] to-[#1E0A20]/20 blur-[2px]"></div>
          ...
          {data.map((entry, idx) => {
            ...
            return (
              ...
              >
                ...
                <div className="h-6 w-px bg-stone-700 absolute top-[30%] -translate-y-[calc(100%+8px)] group-hover:bg-[#1E0A20] transition-colors"></div>

                {/* Point d'ancrage */}
                <div 
                  className="w-4 h-4 rounded-none-full border-2 absolute top-[30%] -translate-y-1/2 -ml-2 bg-[#111] transition-all duration-300 shadow-[0_0_15px_rgba(0,0,0,0.5)] z-20 group-hover:scale-125 group-hover:shadow-[0_0_15px_rgba(184,150,62,0.8)]"
                  style={{ borderColor: color }}
                />

                {/* Carte au-dessus */}
                <div className="absolute bottom-[calc(70%+16px)] bg-stone-900/80 backdrop-blur-sm border border-stone-800 group-hover:border-[#1E0A20]/50 rounded-none p-3 text-center shadow-lg transition-transform group-hover:-translate-y-2 flex flex-col items-center w-full">
                ...
                {/* Détails en-dessous */}
                <div className="absolute top-[calc(30%+24px)] w-full outline outline-1 outline-stone-800 bg-[#1A1A1A] p-4 text-xs text-stone-300 font-[family-name:var(--font-jetbrains)] flex flex-col gap-2 rounded-none transition-all group-hover:bg-[#111111] group-hover:outline-[#1E0A20]/30">
                  ...
                  {d.terme_sheet && (
                    <div className="flex items-start gap-2 border-t border-stone-800 pt-2">
                      <DollarSign className="w-3.5 h-3.5 mt-0.5 text-[#1E0A20] shrink-0" />
                      <div>
