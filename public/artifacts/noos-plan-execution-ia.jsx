...
function App() {
  ...
  const overviewView = h('div', null,
    h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:20 } },
      ...[ ['Total Agents',totalAgents,C.noir],['Poles',6,C.gold],['Daily Output','20,400+',C.green] ].map(([l,v,c],i) =>
        h('div', { key:i, onMouseEnter:()=>setHover('ov-'+i), onMouseLeave:()=>setHover(null),
          style:{ background: hover==='ov-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius:10, padding:20, textAlign:'center', transition:'all 0.2s' }
        },
    ...
    h('div', { style:{ display:'flex', height:32, borderRadius:16, overflow:'hidden', marginBottom:16 } },
    ...
    h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 } },
      ...poles.map((p,i) => h('div', { key:i, onMouseEnter:()=>setHover('p-'+i), onMouseLeave:()=>setHover(null),
        ...
        style:{ background: hover==='p-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius:8, padding:14,
  ...
  const byPoleView = h('div', { style:{ display:'flex', flexDirection:'column', gap:16 } },
    ...poles.map((p,i) => h('div', { key:i, onMouseEnter:()=>setHover('bp-'+i), onMouseLeave:()=>setHover(null),
      style:{ background: hover==='bp-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius:10, padding:20,
      ...
      h('div', { style:{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 } },
        ...
        h('div', { style:{ fontFamily:MF, fontSize:10, padding:'3px 10px', borderRadius:12,
      ...
      h('div', { style:{ display:'flex', flexWrap:'wrap', gap:6 } },
        ...p.tasks.map((t,j) => h('span', { key:j, style:{ fontFamily:MF, fontSize:10, padding:'4px 10px', borderRadius:4, background:p.color+'15', color:C.t1 } }, t))
      )
    ))
  );

  const outputView = h('div', null,
    h('div', { style:{ background:C.cream, borderRadius:10, padding:20, marginBottom:16 } },
      ...
      ...poles.map((p,i) => h('div', { key:i, style:{ display:'flex', alignItems:'center', gap:10, marginBottom:10 } },
        ...
        h('div', { style:{ flex:1, height:10, background:C.div, borderRadius:5, overflow:'hidden' } },
          h('div', { style:{ width:(p.agents/160*100)+'%', height:'100%', background:p.color, borderRadius:5 } })
        ),
    ...
    h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:12 } },
      ...[ ['Weekly Total','142,800+',C.noir], ['Monthly Total','612,000+',C.gold], ['Cost per Task','0.003 EUR',C.green], ['Avg Response','1.2s','#3B82F6'] ].map(([l,v,c],i) =>
        h('div', { key:i, style:{ background:C.ivory, border:'1px solid '+C.div, borderRadius:8, padding:16, textAlign:'center' } },
  ...
  const perfView = h('div', { style:{ display:'flex', flexDirection:'column', gap:12 } },
    h('div', { style:{ display:'grid', gridTemplateColumns:'120px repeat(3,1fr)', gap:8, padding:'8px 12px', background:C.cream, borderRadius:6 } },
    ...
    ...poles.map((p,i) => h('div', { key:i, onMouseEnter:()=>setHover('pf-'+i), onMouseLeave:()=>setHover(null),
      style:{ display:'grid', gridTemplateColumns:'120px repeat(3,1fr)', gap:8, padding:'12px', alignItems:'center',
        background: hover==='pf-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius:6, transition:'all 0.2s' }
    },
      ...
      h('div', null,
        ...
        h('div', { style:{ height:4, background:C.div, borderRadius:2, marginTop:4, overflow:'hidden' } },
          h('div', { style:{ width:p.uptime+'%', height:'100%', background: p.uptime>=99 ? C.green : C.gold, borderRadius:2 } })
        )
      ),
      h('div', null,
        ...
        h('div', { style:{ height:4, background:C.div, borderRadius:2, marginTop:4, overflow:'hidden' } },
          h('div', { style:{ width:p.efficiency+'%', height:'100%', background: p.efficiency>=90 ? C.green : C.gold, borderRadius:2 } })
        )
      ),
      h('div', { style:{ fontFamily:MF, fontSize:10, padding:'3px 10px', borderRadius:12, display:'inline-block',
