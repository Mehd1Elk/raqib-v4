...
function App() {
  ...
  const compView = h('div', null,
    h('div', { style:{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:20 } },
      h('div', { style:{ textAlign:'center', padding:16, background:C.cream, borderRadius: 0, border:'2px solid '+C.green } },
      ...
      h('div', { style:{ textAlign:'center', padding:16, background:C.cream, borderRadius: 0, border:'2px solid '+C.gold } },
        h('div', { style:{ fontSize:28 } }, '\uD83E\uDD16'),
    ...
    h('div', { style:{ display:'flex', flexDirection:'column', gap:6 } },
      h('div', { style:{ display:'grid', gridTemplateColumns:'130px 1fr 1fr 100px', gap:8, padding:'8px 12px', background:C.cream, borderRadius: 0, } },
      ...
      ...metrics.map((m,i) => h('div', { key:i, onMouseEnter:()=>setHover('m-'+i), onMouseLeave:()=>setHover(null),
        style:{ display:'grid', gridTemplateColumns:'130px 1fr 1fr 100px', gap:8, padding:'10px 12px', alignItems:'center',
          background: hover==='m-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius: 0, transition:'all 0.2s' }
      },
  ...
  const ecoView = h('div', null,
    h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16, marginBottom:20 } },
      ...[ ['Interns (25)', economics.intern, C.green], ['AI Agents (800)', economics.ai, C.gold], ['Hybrid Model', null, '#8B5CF6'] ].map(([label, data, color], i) =>
        h('div', { key:i, onMouseEnter:()=>setHover('eco-'+i), onMouseLeave:()=>setHover(null),
          style:{ background: hover==='eco-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius: 0, padding:20,
    ...
    h('div', { style:{ background:C.cream, borderRadius: 0, padding:20 } },
      ...
      ...[ ['All Interns (25)', 624000, 100], ['All AI (800)', 220800, 35], ['Hybrid (25+800)', 220800+52000, 44] ].map(([l,v,pct],i) =>
        h('div', { key:i, style:{ display:'flex', alignItems:'center', gap:10, marginBottom:10 } },
          ...
          h('div', { style:{ flex:1, height:10, background:C.div, borderRadius: 0, overflow:'hidden' } },
            h('div', { style:{ width:pct+'%', height:'100%', background: i===2 ? '#8B5CF6' : i===1 ? C.gold : C.green, borderRadius: 0, } })
          ),
  ...
  const recView = h('div', null,
    h('div', { style:{ textAlign:'center', padding:20, background:C.cream, borderRadius: 0, marginBottom:20 } },
    ...
    h('div', { style:{ display:'flex', flexDirection:'column', gap:12 } },
      ...recommendations.map((r,i) => h('div', { key:i, onMouseEnter:()=>setHover('rec-'+i), onMouseLeave:()=>setHover(null),
        style:{ display:'flex', gap:16, background: hover==='rec-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius: 0, padding:16,
          transition:'all 0.2s', borderLeft:'4px solid '+(prioColors[r.priority]||C.t3) }
      },
        h('div', { style:{ width:36, height:36, borderRadius: 0, background:(prioColors[r.priority]||C.t3)+'22',
