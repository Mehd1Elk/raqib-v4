...
function App() {
  ...
  const structureView = h('div', null,
    h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:12, marginBottom:16 } },
      ...Object.entries(structure).map(([k,v],i) => h('div', { key:i, onMouseEnter:()=>setHover('s-'+i), onMouseLeave:()=>setHover(null),
        style:{ background: hover==='s-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius: 0, padding:16, transition:'all 0.2s' }
      },
    ...
    h('div', { style:{ background:C.cream, border:'1px solid '+C.div, borderRadius: 0, padding:20, textAlign:'center', marginBottom:16 } },
      ...
      h('div', { style:{ display:'flex', justifyContent:'center', alignItems:'center', gap:12 } },
        h('div', { style:{ fontFamily:HF, fontSize:15, fontWeight:700, color:C.gold, padding:'10px 18px', border:'2px solid '+C.gold, borderRadius: 0, background:C.gold+'08' } }, 'Eigen Holding SAS'),
        h('div', { style:{ fontFamily:MF, fontSize:20, color:C.t3 } }, '\u2192'),
        h('div', { style:{ fontFamily:HF, fontSize:15, fontWeight:700, color:C.noir, padding:'10px 18px', border:'2px solid '+C.noir, borderRadius: 0, } }, 'NOOS SAS'),
      ),
      h('div', { style:{ fontFamily:MF, fontSize:10, color:C.t3, marginTop:8 } }, '80% ownership | Full operational control')
    ),
    h('div', { style:{ background:C.ivory, border:'1px solid '+C.div, borderRadius: 0, padding:16 } },
      ...
      ...timeline.map((t,i) => {
        ...
        return h('div', { key:i, style:{ display:'flex', alignItems:'center', gap:12, padding:'8px 0', borderBottom: i<timeline.length-1 ? '1px solid '+C.div+'66' : 'none' } },
          h('div', { style:{ width:10, height:10, borderRadius: 0, background:sc[t.status], flexShrink:0 } }),
          ...
          h('div', { style:{ fontFamily:MF, fontSize:9, padding:'2px 8px', borderRadius: 0, background:sc[t.status]+'22', color:sc[t.status], textTransform:'uppercase' } }, t.status)
        );
      })
    )
  );
  ...
  const articlesView = h('div', { style:{ display:'flex', flexDirection:'column', gap:6 } },
    ...
    ...articles.map((a,i) => h('div', { key:i, onClick:()=>setExpandedArt(expandedArt===i?null:i),
      ...
      style:{ background: hover==='a-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius: 0, padding:14,
  ...
  const shareholdersView = h('div', null,
    h('div', { style:{ display:'flex', height:28, borderRadius: 0, overflow:'hidden', marginBottom:20 } },
    ...
    h('div', { style:{ display:'flex', flexDirection:'column', gap:12, marginBottom:16 } },
      ...shareholders.map((s,i) => h('div', { key:i, onMouseEnter:()=>setHover('shd-'+i), onMouseLeave:()=>setHover(null),
        ...
        h('div', { style:{ display:'flex', alignItems:'center', gap:12 } },
          h('div', { style:{ width:16, height:16, borderRadius: 0, background:s.color } }),
          h('div', null,
    ...
    h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 } },
      ...[ ['Par Value','10 EUR',C.gold], ['Total Shares','5,000',C.noir], ['Capital','50,000 EUR',C.green] ].map(([l,v,c],i) =>
        h('div', { key:i, style:{ background:C.cream, borderRadius: 0, padding:14, textAlign:'center' } },
  ...
  return h('div', { style:{ background:C.ivory, minHeight:'100vh', padding:32 } },
    h('div', { style:{ maxWidth:820, margin:'0 auto' } },
      h('div', { style:{ display:'flex', alignItems:'center', gap:12, marginBottom:4 } },
        ...
        h('span', { style:{ fontFamily:MF, fontSize:10, padding:'3px 10px', borderRadius: 0, background:C.gold+'22', color:C.gold } }, 'v1')
      ),
