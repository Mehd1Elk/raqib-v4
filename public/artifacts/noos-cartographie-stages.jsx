...
function App() {
  ...
  const statusFilter = h('div', { style:{ display:'flex', gap:6, marginBottom:12, alignItems:'center' } },
    ...['all',...statuses].map(s => h('button', { key:s, onClick:()=>setFilter(s),
      style:{ padding:'4px 12px', fontFamily:MF, fontSize:10, borderRadius: 0, cursor:'pointer', transition:'all 0.2s',
    ...
    h('input', { placeholder:'Search title or skill...', value:searchText,
      ...
      style:{ marginLeft:'auto', padding:'6px 12px', fontFamily:MF, fontSize:10, border:'1px solid '+C.div,
        borderRadius: 0, background:C.ivory, color:C.noir, outline:'none', width:180 }
    })
  );
  ...
  const allView = h('div', null, statusFilter,
    ...
    h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:10 } },
      ...filtered.map((intern,i) => h('div', { key:i, onMouseEnter:()=>setHover('i-'+i), onMouseLeave:()=>setHover(null),
        style:{ background: hover==='i-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius: 0, padding:14,
        ...
        h('div', { style:{ display:'flex', flexWrap:'wrap', gap:4, marginBottom:8 } },
          ...intern.skills.map((s,j) => h('span', { key:j, style:{ fontFamily:MF, fontSize:9, padding:'2px 6px', borderRadius: 0, background:C.div+'88', color:C.t1 } }, s))
        ),
        h('div', { style:{ fontFamily:MF, fontSize:10, padding:'2px 8px', borderRadius: 0, display:'inline-block',
  ...
  const byDeptView = h('div', { style:{ display:'flex', flexDirection:'column', gap:16 } },
    ...DEPTS.map(d => {
      ...
      return h('div', { key:d, style:{ background:C.ivory, border:'1px solid '+C.div, borderRadius: 0, padding:16 } },
        h('div', { style:{ display:'flex', alignItems:'center', gap:8, marginBottom:8 } },
          h('div', { style:{ width:12, height:12, borderRadius: 0, background:deptColors[d] } }),
          ...
          openCount > 0 && h('span', { style:{ fontFamily:MF, fontSize:9, padding:'2px 8px', borderRadius: 0, background:C.green+'22', color:C.green, marginLeft:4 } }, openCount+' open')
        ),
        h('div', { style:{ display:'flex', height:6, borderRadius: 0, overflow:'hidden', marginBottom:10 } },
        ...
        h('div', { style:{ display:'flex', flexWrap:'wrap', gap:8 } },
          ...deptInterns.map((x,j) => h('div', { key:j, onMouseEnter:()=>setHover('bd-'+d+j), onMouseLeave:()=>setHover(null),
            style:{ fontFamily:MF, fontSize:11, padding:'6px 10px', borderRadius: 0,
  ...
  const statsView = h('div', { style:{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:16 } },
    ...[['Total Positions',stats.total,C.noir],['Open',stats.open,C.green],['Filled',stats.filled,C.t3],['Planned',stats.planned,C.gold]].map(([l,v,c],i) =>
      h('div', { key:i, onMouseEnter:()=>setHover('st-'+i), onMouseLeave:()=>setHover(null),
        style:{ background: hover==='st-'+i ? C.cream : C.ivory, border:'1px solid '+C.div, borderRadius: 0, padding:20, textAlign:'center', transition:'all 0.2s' }
      },
    ...
    h('div', { style:{ background:C.ivory, border:'1px solid '+C.div, borderRadius: 0, padding:20 } },
      ...
      ...DEPTS.map(d => { const count = internships.filter(x=>x.dept===d).length; return h('div', { key:d, style:{ display:'flex', alignItems:'center', gap:10, marginBottom:8 } },
        ...
        h('div', { style:{ flex:1, height:8, background:C.div, borderRadius: 0, overflow:'hidden' } },
          h('div', { style:{ width:(count/25*100)+'%', height:'100%', background:deptColors[d], borderRadius: 0, } })
        ),
